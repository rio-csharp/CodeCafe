...
public sealed class NotesMcpResources
{
    ...
    public TextResourceContents GetGuideResource()
    {
        var text = string.Join(Environment.NewLine, new[]
        {
            ...
            $"- Prefer that HTTP upload path for large Markdown so the file contents do not have to travel through MCP tool arguments. After upload, pass the returned upload id into `{NotesMcpToolNames.CreatePage}`, `{NotesMcpToolNames.UpdatePageContentJson}`, or `{NotesMcpToolNames.AppendBlocksToPage}`.",
