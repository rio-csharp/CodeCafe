    ...
    public static string? GetSuggestion(string code)
    {
        return code switch
        {
            "content_too_large" or "upload_too_large" or "upload_chunk_too_large" or "invalid_content_json" or "invalid_blocks" or "markdown_conversion_failed" or "invalid_tiptap_document"
                => $"Call {NotesMcpToolNames.GetLimits} to inspect MCP byte and TipTap node/text limits. For larger content, use {NotesMcpToolNames.CreateUpload} and {NotesMcpToolNames.AppendUploadChunk}, then pass the upload id to the page tool.",
