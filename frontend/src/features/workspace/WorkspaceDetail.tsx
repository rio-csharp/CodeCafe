      ...
      <aside className="flex w-[200px] shrink-0 flex-col border-r border-border bg-gradient-to-b from-[rgba(17,26,44,0.94)] to-[rgba(9,14,25,0.96)] shadow-[inset_-1px_0_0_rgba(56,189,248,0.08)]">
        ...
        <div className="flex items-center gap-2 px-3 py-3.5">
          <svg aria-hidden="true" viewBox="0 0 32 32" width="22" height="22">
            <rect width="32" height="32" rx="7" fill="url(#wsd-logo-grad)" />
            ...
            <defs>
              <linearGradient id="wsd-logo-grad" x1="0" y1="0" x2="32" y2="32">
        ...
        <nav className="flex flex-1 flex-col gap-1 px-3">
          <SidebarNavItem icon={<OverviewIcon />} label="Overview" active />
          <SidebarNavItem icon={<ChatIcon />} label="Chat" />
          <SidebarNavItem icon={<CodeIcon />} label="Code" />
          <SidebarNavItem icon={<RunsIcon />} label="Runs" />
          <SidebarNavItem icon={<NotesIcon />} label="Notes" />
        </nav>
...
function SidebarNavItem({ icon, label, active }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <div className={`flex items-center gap-2.5 rounded-lg px-3 py-2 text-sm font-medium transition-colors ${
      active ? 'bg-accent/10 text-text' : 'text-muted hover:bg-accent/8 hover:text-text'
    }`}>
      {icon}
      <span>{label}</span>
    </div>
