...
import { ChevronDown, ChevronRight, ListPlus, Loader2, PenLine, Plus, Sparkles } from 'lucide-react'
...
export function DraftWorkspace({
  ...
  const [isExpanded, setIsExpanded] = useState(!hasConversation)

  // Auto-expand when a draft is generated or received.
  // Wrapped in rAF to avoid the synchronous-setState-in-effect lint rule.
  const didAutoExpandRef = useRef(false)
  useEffect(() => {
    if (draft && !didAutoExpandRef.current) {
