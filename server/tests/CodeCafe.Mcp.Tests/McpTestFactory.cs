...
public sealed class McpTestFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        {
            services
                .AddAuthentication(McpTestAuthHandler.SchemeName)
            ...
            services.AddScoped<IUploadStore>(serviceProvider =>
                serviceProvider.GetRequiredService<TestMcpUploadStore>()
            );
...
internal sealed class TestNotebookQueryService : INotebookReadService
{
    ...
    private static readonly JsonElement OverviewContentJson = JsonSerializer.SerializeToElement(
        new
        {
            type = "doc",
            content = new[]
    ...
    public Task<NotesResult<NotebookItemModel>> GetPublicNotebookItemAsync(
        ...
        Task.FromResult(
            ...
            && Items.SingleOrDefault(item =>
                string.Equals(
                    item.Path,
                    NotebookInput.NormalizePath(path),
    ...
    )
    {
        if (notebookId != NotebookId)
        {
            return Task.FromResult(
                NotesResult<NotebookItemsPageModel>.Failure(
                    NotesFailureKind.NotFound,
                    "notebook_not_found",
                    "Notebook was not found."
                )
        ...
        var pagedItems = filteredItems
            ...
            .ToList();
        return Task.FromResult(
            NotesResult<NotebookItemsPageModel>.Success(
...
internal sealed class TestNotebookMutationService : INotebookItemMutationService
{
    ...
    public Task<NotesResult<NotebookItemModel>> ArchiveNotebookItemAsync(
        ...
        Task.FromResult(
            NotesResult<NotebookItemModel>.Success(
                CreateItem(itemId, notebookId, null, "page", "Archived", 1, null)
            )
        );
    ...
    public Task<NotesResult<NotebookItemModel>> RestoreNotebookItemAsync(
        ...
        Task.FromResult(
            NotesResult<NotebookItemModel>.Success(
                CreateItem(itemId, notebookId, null, "page", "Restored", 1, null)
            )
        );
...
internal sealed class TestMcpUploadStore : IUploadStore
{
...
internal sealed class NoopMcpAuditService : IMcpAuditService
{
    ...
    public Task WriteIndependentAsync(
