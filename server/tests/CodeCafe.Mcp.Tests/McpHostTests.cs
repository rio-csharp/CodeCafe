...
public sealed class McpHostTests : IClassFixture<McpTestFactory>
{
    ...
    public async Task McpToolsResourcesAndPrompts_AreExposedByTheStandaloneHost()
    {
        ...
        client.DefaultRequestHeaders.Add(
            McpTestAuthHandler.UserIdHeader,
        ...
        var createPageTool = Assert.Single(
            tools,
            tool => tool.Name == NotesMcpToolNames.CreatePage
        );
        var updatePageTool = Assert.Single(
            tools,
            tool => tool.Name == NotesMcpToolNames.UpdatePageContent
        );
        var renameItemTool = Assert.Single(
            tools,
            tool => tool.Name == NotesMcpToolNames.RenameItem
        );
        ...
        Assert.Contains(
            "maxPageContentBytes",
            createPageTool.Description ?? string.Empty,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "maxTipTapNodeCount",
            updatePageTool.Description ?? string.Empty,
            StringComparison.Ordinal
        );
        Assert.Contains(
            "maxTipTapDepth",
        ...
        Assert.Contains(
            "page/<path>",
            renameItemTool.Description ?? string.Empty,
            StringComparison.Ordinal
        );
        ...
        var diagnosticsResult = await mcpClient.CallToolAsync(
            "diagnostics_status",
        ...
        Assert.Equal(
            "ok",
            diagnosticsResult.StructuredContent!.Value.GetProperty("status").GetString()
        );
        Assert.Equal(
            "mcp",
            diagnosticsResult.StructuredContent!.Value.GetProperty("adapter").GetString()
        );
        ...
        var limitsResult = await mcpClient.CallToolAsync(
            NotesMcpToolNames.GetLimits,
            new Dictionary<string, object?>()
        );
        Assert.Equal(
            131072,
            limitsResult.StructuredContent!.Value.GetProperty("maxInlineContentBytes").GetInt32()
        );
        Assert.Equal(
            4194304,
            limitsResult.StructuredContent!.Value.GetProperty("maxHttpUploadBytes").GetInt32()
        );
        Assert.Equal(
            900,
            limitsResult.StructuredContent!.Value.GetProperty("uploadIdleTimeoutSeconds").GetInt32()
        );
        Assert.Equal(
            64,
            limitsResult.StructuredContent!.Value.GetProperty("maxTipTapDepth").GetInt32()
        );
        Assert.Equal(
            5000,
            limitsResult.StructuredContent!.Value.GetProperty("maxTipTapNodeCount").GetInt32()
        );
        Assert.Equal(
            200000,
            limitsResult.StructuredContent!.Value.GetProperty("maxTipTapTextLength").GetInt32()
        );

        var httpUploadResult = await mcpClient.CallToolAsync(
            NotesMcpToolNames.PrepareHttpUpload,
            new Dictionary<string, object?>()
        );
        Assert.Equal(
            "/api/mcp/uploads/markdown",
            httpUploadResult.StructuredContent!.Value.GetProperty("uploadUrl").GetString()
        );
        Assert.Equal(
            "POST",
            httpUploadResult.StructuredContent!.Value.GetProperty("method").GetString()
        );
        Assert.Equal(
            "multipart/form-data",
            httpUploadResult.StructuredContent!.Value.GetProperty("contentType").GetString()
        );
        Assert.Equal(
            4194304,
            httpUploadResult.StructuredContent!.Value.GetProperty("maxUploadBytes").GetInt32()
        );
        Assert.Contains(
            NotesMcpToolNames.CreatePage,
            httpUploadResult.StructuredContent!.Value.GetProperty("nextStep").GetString(),
            StringComparison.Ordinal
        );
        ...
        Assert.Equal(
            "Architecture Notes",
            detailResult.StructuredContent!.Value.GetProperty("title").GetString()
        );
    }
    ...
    public async Task RenameItem_AcceptsResourceStylePagePrefixForBareStoredPath()
    {
        ...
        var renameResult = await mcpClient.CallToolAsync(
        ...
        Assert.Equal(
            "Renamed Overview",
            renameResult.StructuredContent!.Value.GetProperty("title").GetString()
        );
    }
    ...
    public async Task McpUpload_AllowsChunksLargerThanTheOldDefaultLimit()
    {
        ...
        var appendResult = await mcpClient.CallToolAsync(
            ...
            new Dictionary<string, object?> { ["uploadId"] = uploadId, ["chunkText"] = chunkText }
        );

        Assert.False(appendResult.IsError ?? false);
        Assert.Equal(
            20_000,
            appendResult.StructuredContent!.Value.GetProperty("bytesReceived").GetInt32()
        );
    }
    ...
    public async Task DiscardUpload_IsIdempotent()
    {
        ...
        var firstDiscard = await mcpClient.CallToolAsync(
            NotesMcpToolNames.DiscardUpload,
            new Dictionary<string, object?> { ["uploadId"] = uploadId }
        );
        var secondDiscard = await mcpClient.CallToolAsync(
            NotesMcpToolNames.DiscardUpload,
            new Dictionary<string, object?> { ["uploadId"] = uploadId }
        );
        ...
        Assert.Equal(
            "discarded",
            firstDiscard.StructuredContent!.Value.GetProperty("result").GetString()
        );
        Assert.False(secondDiscard.IsError ?? false);
        Assert.Equal(
            "already_absent",
            secondDiscard.StructuredContent!.Value.GetProperty("result").GetString()
        );
    }
    ...
    {
        var result = NotesMcpResultMapper.Failure(
            new NotesError(
        ...
        Assert.Equal(
            5000,
            content.GetProperty("details").GetProperty("maxTipTapNodeCount").GetInt32()
        );
        Assert.Equal(
            5001,
            content.GetProperty("details").GetProperty("actualTipTapNodeCount").GetInt32()
        );
    }
    ...
    public async Task PrecisionEditTools_CanMutatePageContent()
    {
        ...
        var replaceResult = await mcpClient.CallToolAsync(
            ...
            new Dictionary<string, object?>
            {
                ...
                ["path"] = "overview",
                ["index"] = 0,
                ["block"] = JsonSerializer.SerializeToElement(
                    new
                    {
                        type = "heading",
                        attrs = new { level = 1 },
                        content = new[] { new { type = "text", text = "New heading" } },
                    }
                ),
            }
        );
        ...
        Assert.Equal(
            5,
            replaceResult.StructuredContent!.Value.GetProperty("tipTapNodeCount").GetInt32()
        );

        var insertResult = await mcpClient.CallToolAsync(
            ...
            new Dictionary<string, object?>
            {
                ...
                ["path"] = "overview",
                ["index"] = 1,
                ["blocks"] = JsonSerializer.SerializeToElement(
                    new[]
                    {
                        new
                        {
                            type = "paragraph",
                            content = new[] { new { type = "text", text = "Inserted paragraph." } },
                        },
                    }
                ),
            }
        ...
        Assert.Equal(
            7,
            insertResult.StructuredContent!.Value.GetProperty("tipTapNodeCount").GetInt32()
        );

        var deleteResult = await mcpClient.CallToolAsync(
        ...
        Assert.Equal(
            3,
            deleteResult.StructuredContent!.Value.GetProperty("tipTapNodeCount").GetInt32()
        );

        var replaceTextResult = await mcpClient.CallToolAsync(
    ...
    public async Task CreatePage_RejectsTitleLongerThan160Characters()
    {
        ...
        Assert.Equal(
            "validation_error",
            createResult.StructuredContent!.Value.GetProperty("code").GetString()
        );
    }
}
