...
public sealed class ApiTestFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureServices(services =>
        {
            ...
            services.AddScoped<INotebookMutationStore>(serviceProvider =>
                serviceProvider.GetRequiredService<TestNotebookMutationStore>()
            );
...
internal sealed class TestNotebookQueryService(TestNotebookMutationStore notebookMutationStore)
    : INotebookReadService
{
        ...
        Task.FromResult(
            string.Equals(slug, "architecture-notes", StringComparison.Ordinal)
                ? NotesResult<NotebookDetailModel>.Success(
                    includeItems ? PublicNotebook : PublicNotebook with { Items = [] }
                )
    ...
    public Task<NotesResult<NotebookItemModel>> GetNotebookItemByIdAsync(
        ...
        Task.FromResult(
            notebookId == NotebookId && Items.SingleOrDefault(item => item.Id == itemId) is { } item
...
internal sealed class TestNotebookCommandService : INotebookItemMutationService
{
    ...
    public Task<NotesResult<NotebookItemModel>> CreateNotebookItemAsync(
        ...
        string title,
        int sortOrder,
        System.Text.Json.JsonElement? contentJson,
        CancellationToken cancellationToken
    ) =>
        Task.FromResult(
    ...
    public Task<NotesResult<NotebookItemModel>> UpdateNotebookItemAsync(
        ...
        Guid currentUserId,
        string title,
        System.Text.Json.JsonElement parentId,
        int? sortOrder,
        System.Text.Json.JsonElement contentJson,
        CancellationToken cancellationToken,
...
internal sealed class TestNotebookMutationStore : INotebookMutationStore
{
    ...
    public Task<NotebookFavorite?> GetFavoriteAsync(
        ...
        Task.FromResult(
            _favorites.SingleOrDefault(favorite =>
                favorite.NotebookId == notebookId && favorite.UserId == currentUserId
            )
        );
    ...
    )
    {
        ...
        if (
            _notebooks.Values.All(notebook =>
                notebook.Id == currentNotebookId || notebook.Slug != baseSlug
            )
        )
        {
