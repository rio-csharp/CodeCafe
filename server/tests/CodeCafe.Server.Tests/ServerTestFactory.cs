...
public sealed class ServerTestFactory : WebApplicationFactory<Program>
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        ...
        builder.ConfigureAppConfiguration(
            (_, configurationBuilder) =>
            {
                ...
                configurationBuilder.AddInMemoryCollection(
                    new Dictionary<string, string?> { ["Auth:RegistrationEnabled"] = "true" }
                );
            }
        );
        ...
        {
            services
                .AddAuthentication(ServerTestAuthHandler.SchemeName)
            ...
            services.AddSingleton<INotebookMutationStore>(serviceProvider =>
                serviceProvider.GetRequiredService<ServerTestNotebookMutationStore>()
            );
            ...
            services.AddScoped<IUploadStore>(serviceProvider =>
                serviceProvider.GetRequiredService<ServerTestMcpUploadStore>()
            );
            services.RemoveAll<IAiNotebookEditProposalStore>();
            services.AddSingleton<
                IAiNotebookEditProposalStore,
                MemoryAiNotebookEditProposalStore
            >();
        });
    }
}
...
) : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    {
        if (
            !Request.Headers.TryGetValue(UserIdHeader, out var userIdValues)
...
internal sealed class ServerTestNotebookQueryService(
    ServerTestNotebookMutationStore notebookMutationStore
) : INotebookReadService
{
            ...
            notebookMutationStore
                ...
                .Where(item =>
                    ...
                    && (
                        ...
                        || (
                            item.PlainTextContent?.Contains(
                                search,
    ...
    public Task<NotesResult<NotebookDetailModel>> GetPublicNotebookAsync(
        ...
        Task.FromResult(
            ...
            && notebookMutationStore.TryGetNotebookDetail(
                NotebookId,
                currentUserId,
                out var notebookDetail
            )
    ...
    public Task<NotesResult<NotebookItemModel>> GetPublicNotebookItemAsync(
        ...
        Task.FromResult(
            string.Equals(slug, "architecture-notes", StringComparison.Ordinal)
                ? NotesResult<NotebookItemModel>.Success(
                    notebookMutationStore.GetItems().Single(item => item.Path == path)
                )
    ...
    public Task<NotesResult<NotebookDetailModel>> GetNotebookBySlugAsync(
        ...
        Task.FromResult(
            notebookMutationStore.TryGetNotebookDetailBySlug(
                slug,
                currentUserId,
                out var notebookDetail
            )
    ...
    public Task<NotesResult<NotebookItemModel>> GetNotebookItemByIdAsync(
        ...
        Task.FromResult(
            notebookId == NotebookId
            && notebookMutationStore.GetItems().SingleOrDefault(item => item.Id == itemId)
                is { } item
...
internal sealed class ServerTestNotebookCommandService(
    ServerTestNotebookMutationStore notebookMutationStore
) : INotebookItemMutationService
{
    ...
    {
        var existingItem = notebookMutationStore
            .GetItems()
    ...
    public Task<NotesResult<IReadOnlyList<NotebookItemModel>>> ReorderNotebookItemsAsync(
        ...
        Task.FromResult(
            NotesResult<IReadOnlyList<NotebookItemModel>>.Failure(
                ...
                "not_implemented",
                "Not implemented in server tests."
            )
        );

    public Task<NotesResult> DeleteNotebookItemAsync(
        Guid notebookId,
        Guid itemId,
        Guid currentUserId,
        CancellationToken cancellationToken
    ...
    public Task<NotesResult<NotebookItemModel>> ArchiveNotebookItemAsync(
        Guid notebookId,
        Guid itemId,
        Guid currentUserId,
        CancellationToken cancellationToken
    ...
    {
        var item = notebookMutationStore
            .GetItems()
    ...
    public Task<NotesResult<NotebookItemModel>> RestoreNotebookItemAsync(
        Guid notebookId,
        Guid itemId,
        Guid currentUserId,
        CancellationToken cancellationToken
...
{
    private static readonly Guid DefaultNotebookId = Guid.Parse(
        "11111111-1111-1111-1111-111111111111"
    );
    private static readonly Guid DefaultOwnerId = Guid.Parse(
    ...
    public bool TryGetNotebookDetail(
        Guid notebookId,
        Guid currentUserId,
        out NotebookDetailModel notebookDetail
    )
    {
    ...
    public bool TryGetNotebookDetailBySlug(
        string slug,
        Guid currentUserId,
        out NotebookDetailModel notebookDetail
    )
    {
        var notebook = _notebooks.Values.SingleOrDefault(existingNotebook =>
            string.Equals(existingNotebook.Slug, slug, StringComparison.Ordinal)
        );
...
internal sealed class ServerTestAuthUserGateway : IAuthUserGateway
{
    ...
    )
    {
        AuthUserModel? user = normalizedEmail switch
        {
            ...
            "existing.user@example.com" => new AuthUserModel(
                DefaultUserId,
                normalizedEmail,
                "Existing User"
            ),
    ...
    public Task<AuthUserModel?> FindByIdAsync(Guid userId, CancellationToken cancellationToken)
    {
        return Task.FromResult<AuthUserModel?>(
            userId == Guid.Empty ? null : new AuthUserModel(userId, "yao@example.com", "Yao")
        );
    }
        ...
        return Task.FromResult(
            string.Equals(normalizedEmail, "existing.user@example.com", StringComparison.Ordinal)
                ...
                : AuthCreateUserResult.Success(
                    new AuthUserModel(DefaultUserId, normalizedEmail, displayName)
                )
        );
    }
...
{
    private readonly ConcurrentDictionary<string, UploadState> uploads = new(
    ...
    )
    {
        ...
        {
            return Task.FromResult(
                UploadResult<UploadStatus>.Failure(
    ...
    )
    {
        ...
        {
            return Task.FromResult(
                UploadResult<UploadSession>.Success(
                    new UploadSession(
