...
public sealed class NotebookItemTreeTests
{
    ...
    public void GeneratePath_Falls_Back_To_Unique_Slug_After_Ten_Conflicts()
    {
        ...
        Assert.StartsWith("hello-world-", path);
        Assert.DoesNotContain(items, item => item.Path.Value == path);
    }
    ...
    public void ApplyDescendantPathUpdate_Rewrites_Children()
    {
        ...
        Assert.Equal("new-root/child", items[1].Path.Value);
        Assert.Equal("new-root/child/grand-child", items[2].Path.Value);
    }
    ...
    public void ApplyDescendantPathUpdate_Rewrites_Subtree_On_Folder_Move()
    {
        ...
        Assert.Equal("archive/folder/page", items[2].Path.Value);
        Assert.Equal("archive", items[1].Path.Value);
    }
    ...
    )
    {
        ...
        if (archived)
        {
            item.Archive(DateTimeOffset.UtcNow, Guid.NewGuid());
        }
