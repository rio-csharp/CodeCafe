...
public sealed partial class NotebookReadService
{
    ...
    )
    {
        ...
        var notebook = await dbContext
            ...
            .SingleAsync(
                existingNotebook => existingNotebook.Id == notebookAccessResult.Value!.Id,
                cancellationToken
            );
    ...
    )
    {
        ...
        var items = await OrderNotebookItems(
                dbContext
    ...
    )
    {
        ...
        if (!UsesPostgresProvider())
        {
            ...
            var summaries = await ToSummaryModelsAsync(
                [notebook],
                currentUserId,
                cancellationToken,
                includeArchived
    ...
    {
        var favoriteCount = await dbContext
