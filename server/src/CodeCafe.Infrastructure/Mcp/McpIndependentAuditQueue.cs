...
internal sealed class McpIndependentAuditQueue(
    ...
    ILogger<McpIndependentAuditQueue> logger
) : BackgroundService, IMcpIndependentAuditQueue
{
    ...
    private readonly Channel<McpAuditRecord> queue = Channel.CreateBounded<McpAuditRecord>(
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (await queue.Reader.WaitToReadAsync(stoppingToken))
        {
            ...
            catch (Exception exception)
            {
                logger.LogWarning(
                    exception,
                    "Failed to flush {Count} queued MCP audit record(s); retrying once after a delay.",
                    batch.Count
                );
                try
                {
                ...
                catch (Exception retryException)
                {
                    logger.LogWarning(
                        retryException,
                        "Dropping {Count} MCP audit record(s) after the flush retry also failed.",
                        batch.Count
                    );
                }
            }
    ...
    private async Task FlushBatchAsync(
        IReadOnlyList<McpAuditRecord> batch,
        CancellationToken cancellationToken
    )
    {
