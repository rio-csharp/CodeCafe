...
public sealed class UploadSessionConfiguration : IEntityTypeConfiguration<UploadSession>
{
    public void Configure(EntityTypeBuilder<UploadSession> entity)
    {
        ...
        entity.Property(session => session.UploadId).HasMaxLength(64).ValueGeneratedNever();

        entity.Property(session => session.ActorUserId).IsRequired();

        entity.Property(session => session.FileName).HasMaxLength(512);

        entity.Property(session => session.MediaType).HasMaxLength(160).IsRequired();

        entity.Property(session => session.BytesReceived).IsRequired();
        ...
        entity
            .HasMany(session => session.Chunks)
