...
public sealed class NotebookItemConfiguration : IEntityTypeConfiguration<NotebookItem>
{
    public void Configure(EntityTypeBuilder<NotebookItem> entity)
    {
        ...
        entity.Property(item => item.Path).HasMaxLength(1024).IsRequired();

        entity.Property(item => item.SortOrder).IsRequired();

        entity.Property(item => item.ContentFormat).HasMaxLength(40);

        entity.Property(item => item.ContentJson).HasColumnType("jsonb");

        entity.Property(item => item.PlainTextContent);

        entity.Property(item => item.Revision).IsConcurrencyToken().IsRequired();
        ...
        // Trigram indexes back the ILIKE '%term%' search in NotebookReadService (PostgreSQL only;
        // other providers ignore the gin method/operator annotations and create plain indexes).
