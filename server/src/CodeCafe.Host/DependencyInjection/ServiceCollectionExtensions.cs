...
public static class ServiceCollectionExtensions
{
    ...
    )
    {
        ...
        services
            ...
            .Validate(
                options => Uri.TryCreate(options.Issuer, UriKind.Absolute, out _),
            ...
            .Validate(
                options =>
                    options.PublicClients.All(client =>
                        !string.IsNullOrWhiteSpace(client.ClientId)
                        && client.RedirectUris.Length > 0
                        && client.RedirectUris.All(uri =>
            ...
            .Validate(
                options =>
                    options.PublicClients.All(client =>
                        OpenIddictClientRegistration.AreAllowedScopesValid(
                            client.AllowedScopes,
                            GetMcpOptions(configuration)
                        )
                    ),
            ...
            .Validate(
                options => !environment.IsProduction() || HasProductionCertificates(options),
        ...
        services
            ...
            .AddServer(options =>
            {
                ...
                var mcpOptions =
                    configuration.GetSection(McpOptions.SectionName).Get<McpOptions>()
                    ?? new McpOptions();
                ...
                options.RegisterAudiences(
                    McpResourceIdentifiers.GetAudienceValues(mcpOptions, authOptions)
                );
                options.RegisterResources(
                    McpResourceIdentifiers.GetResourceValues(mcpOptions, authOptions)
                );
            ...
            .AddValidation(options =>
            {
                ...
                options.AddAudiences(
                    McpResourceIdentifiers.GetAudienceValues(
                        mcpOptions,
                        GetAuthorizationServerOptions(configuration, environment)
                    )
                );
    ...
    )
    {
        ...
        services
            ...
            .Configure<IOptions<ShutdownOptions>>(
                (options, shutdownOptionsAccessor) =>
                {
                    options.ShutdownTimeout = TimeSpan.FromSeconds(
                        shutdownOptionsAccessor.Value.TimeoutSeconds
                    );
                }
            );
    ...
    private static IServiceCollection AddCodeCafeOpenIddictPruningOptions(
        this IServiceCollection services
    )
    {
        services
            .AddOptions<OpenIddictPruningOptions>()
            ...
            .Validate(
                options => options.PruneThresholdDays > 0,
    ...
    )
    {
        ...
        healthChecks.AddCheck<ServerDrainReadinessHealthCheck>("drain", tags: ["ready"]);
    ...
    )
    {
        services
            ...
            .Validate(
                settings => settings.KnownNetworks.All(IsValidNetwork),
    ...
    {
        var dataProtectionBuilder = services.AddDataProtection().SetApplicationName("CodeCafe");
    ...
    private static IServiceCollection AddCodeCafeRateLimiting(this IServiceCollection services)
    {
        services.AddRateLimiter(options =>
        {
            ...
            options.GlobalLimiter = PartitionedRateLimiter.Create<HttpContext, string>(
                httpContext =>
                {
                    var clientIpAddressAccessor =
                        httpContext.RequestServices.GetRequiredService<IClientIpAddressAccessor>();
                    return RateLimitPartition.GetFixedWindowLimiter(
                        GetRateLimitPartitionKey(
                            httpContext,
                            clientIpAddressAccessor,
                            allowAuthenticatedUserKey: true
                        ),
                ...
                httpContext =>
                {
                    ...
                    return RateLimitPartition.GetFixedWindowLimiter(
                        GetRateLimitPartitionKey(
                            httpContext,
                            clientIpAddressAccessor,
                            allowAuthenticatedUserKey: false
                        ),
    ...
    {
        return configuration.GetSection(McpOptions.SectionName).Get<McpOptions>()
            ?? new McpOptions();
    }
    ...
    {
        var clientIpAddressAccessor =
            context.HttpContext.RequestServices.GetRequiredService<IClientIpAddressAccessor>();
        var logger = context
