...
) : ControllerBase
{
    ...
    public async Task<IActionResult> Authorize(CancellationToken cancellationToken)
    {
        var request =
            ...
            ?? throw new InvalidOperationException(
                "The OpenID Connect request cannot be resolved."
            );
        ...
        {
            return BadRequest(
                new OAuthErrorResponse("invalid_client", "The OAuth client id is required.")
            );
        }
        ...
        if (application is null)
        {
            return BadRequest(
                new OAuthErrorResponse(
                    "invalid_client",
                    "The OAuth client application is not registered."
                )
            );
        }

        var requestedScopes = request.GetScopes().ToArray();
        var unauthorizedScopes = await GetUnauthorizedScopesAsync(
            application,
            requestedScopes,
            cancellationToken
        );
        ...
        if (request.IsAuthorizationCodeGrantType() || request.IsRefreshTokenGrantType())
        {
            ...
            if (user is null)
            {
                return Forbid(
                    properties: CreateServerErrorProperties(
                        "invalid_grant",
                        "The token is bound to an account that no longer exists."
                    ),
                    authenticationSchemes: [OpenIddictServerAspNetCoreDefaults.AuthenticationScheme]
                );
            }
    ...
    )
    {
        ...
        identity.SetDestinations(claim =>
            claim.Type switch
            {
                Claims.Subject when includeIdentityToken =>
                [
                    Destinations.AccessToken,
                    Destinations.IdentityToken,
                ],
                Claims.Name when includeIdentityToken && includeProfile =>
                [
                    Destinations.AccessToken,
                    Destinations.IdentityToken,
                ],
                Claims.Email when includeIdentityToken && includeEmail =>
                [
                    Destinations.AccessToken,
                    Destinations.IdentityToken,
                ],
        ...
        await foreach (
            var resource in scopeManager.ListResourcesAsync(
                principal.GetScopes(),
                cancellationToken
            )
        )
    ...
    )
    {
        ...
        var protectedScopes = mcpOptionsAccessor
            .Value.RequiredReadScopes.Concat(mcpOptionsAccessor.Value.RequiredWriteScopes)
    ...
    private static AuthenticationProperties CreateServerErrorProperties(
        string error,
        string description
    )
    {
