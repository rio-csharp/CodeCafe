...
public sealed class NotesMcpNotebookTools
{
    ...
    public async Task<CallToolResult> ListNotebooksAsync(
        ...
        [Description("Optional search query to filter notebooks by title or description.")]
            string? query = null,
    ...
    )
    {
        ...
        {
            notebooks.AddRange(
                await notebookReadService.GetMyNotebooksAsync(
                    currentUserId,
                    query,
                    cancellationToken,
                    maxResults
        ...
        {
            notebooks.AddRange(
                await notebookReadService.GetPublicNotebooksAsync(
                    query,
                    currentUserId,
                    cancellationToken,
                    maxResults
        ...
        var response = new ListNotebooksToolResponse(
            normalizedScope,
            notebookDetails.Count,
            notebookDetails
        );
        return NotesMcpResultMapper.Success(
            response,
            $"Listed {response.TotalCount} notebook(s) for scope '{response.Scope}'."
        );
    }
    ...
    [Description(
        "Read notebook metadata for a notebook the authenticated actor is allowed to access."
    )]
    ...
    )
    {
        ...
        var response = NotesMcpSupport.ToGetNotebookToolResponse(
            notebookContextResult.Value.Notebook
        );
    ...
    [Description(
        "Return inline, upload, page-size, and pagination limits. Call this before sending large content."
    )]
    public CallToolResult GetLimits(IOptions<McpOptions> mcpOptionsAccessor)
    {
    ...
    [Description(
        "For local Markdown files, call this tool FIRST. It returns the HTTP request details needed to upload a file directly via POST /api/mcp/uploads/markdown using the same OAuth bearer token used for MCP. The client must then perform the returned HTTP request with multipart/form-data and pass the returned uploadId to notes_create_page, notes_update_page_content, or notes_append_blocks_to_page. Only fall back to notes_create_upload if the client cannot make HTTP requests."
    )]
    ...
    public async Task<CallToolResult> SearchAsync(
        ...
        CancellationToken cancellationToken,
        IOptions<McpOptions> mcpOptionsAccessor,
        [Description("Optional notebook slug to scope the search to one notebook.")]
            string? notebookSlug = null,
    ...
    )
    {
        ...
        {
            var scopedNotebook = await NotesMcpSupport.RequireNotebookAsync(
                notebookSlug,
                user,
                notebookReadService,
                cancellationToken
            ...
            if (normalizedScope is "all" or "notebooks")
            {
                ...
                {
                    results.Add(
                        new NotebookSearchResultResponse(
                            notebook.Id,
                            notebook.Slug,
                            notebook.Title,
            ...
            if (normalizedScope is "all" or "items")
            {
                ...
                if (remainingResults > 0)
                {
                    ...
                    {
                        results.AddRange(
                            itemResults.Value!.Select(item => new NotebookSearchResultResponse(
                                notebook.Id,
                                notebook.Slug,
                                notebook.Title,
                                NotesMcpSupport.BuildNotebookUri(notebook.Slug),
        ...
        else
        {
            if (normalizedScope is "all" or "notebooks")
            {
                ...
                var myNotebooks = await notebookReadService.GetMyNotebooksAsync(
                    currentUserId,
                    query,
                    cancellationToken,
                    maxResults
                ...
                results.AddRange(
                    summaries.Select(notebook => new NotebookSearchResultResponse(
                        notebook.Id,
                        notebook.Slug,
                        notebook.Title,
                        NotesMcpSupport.BuildNotebookUri(notebook.Slug),
            ...
            if (normalizedScope is "all" or "items")
            {
                ...
                if (remainingResults > 0)
                {
                    ...
                    results.AddRange(
                        itemResults.Select(item => new NotebookSearchResultResponse(
                            item.NotebookId,
                            item.NotebookSlug,
                            item.NotebookTitle,
        ...
        var response = new SearchNotesToolResponse(
            query.Trim(),
            trimmedResults.Count,
            trimmedResults
        );
        return NotesMcpResultMapper.Success(
            response,
            $"Found {response.TotalCount} result(s) for '{response.Query}'."
        );
    }
    ...
    public async Task<CallToolResult> CreateNotebookAsync(
        ...
        [Description("Notebook visibility: private, unlisted, or public. Defaults to private.")]
    ...
    )
    {
        return await mutationExecutor.ExecuteAsync(
            ...
            {
                var mcpOptions = mcpOptionsAccessor.Value;
                var actorResult = NotesMcpSupport.RequireActor(
                    user,
                ...
                if (
                    ...
                    && visibility is not ("public" or "unlisted" or "private")
                )
                {
                    return McpMutationResult<GetNotebookToolResponse>.Failure(
                        new NotesError(
                            NotesFailureKind.Validation,
                            "invalid_visibility",
                ...
                var createResult = await NotesMcpCommandSender.SendAsync(
                    sender,
                    new CreateNotebookCommand(actorResult.Value, title, description, visibility),
                    ct
                );
    ...
    public async Task<CallToolResult> UpdateNotebookAsync(
        ...
        [Description("Optional new notebook description. Use an empty string to clear it.")]
            string? description = null,
        [Description("Optional new notebook visibility: private, unlisted, or public.")]
            string? visibility = null
    )
    {
        return await mutationExecutor.ExecuteAsync(
    ...
    )
    {
        return await mutationExecutor.ExecuteAsync(
            ...
            async ct =>
            {
                ...
                if (!notebookContextResult.Succeeded)
                {
                    return McpMutationResult<DeleteNotebookToolResponse>.Failure(
                        notebookContextResult.Error!
                    );
                }
                ...
                var deleteResult = await NotesMcpCommandSender.SendAsync(
                    sender,
                    new DeleteNotebookCommand(notebook.Id, notebookContext.ActorId),
                    ct
                );
                ...
                var response = new DeleteNotebookToolResponse(
                    notebook.Id,
                    notebook.Slug,
                    "deleted"
                );
