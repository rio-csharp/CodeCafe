...
public sealed partial class NotesMcpItemTools
{
    ...
    [Description(
        "Fallback upload path for clients that cannot use the HTTP upload returned by notes_prepare_http_upload. Creates a server-managed MCP upload session for chunked content such as Markdown or TipTap JSON. Use notes_append_upload_chunk to send the file text, then pass the uploadId to notes_create_page, notes_update_page_content, or notes_append_blocks_to_page."
    )]
    public async Task<CallToolResult> CreateUpload(
        ...
        [Description(
            "Optional original file name, such as notes.md or page.json. Used for format inference."
        )]
            string? fileName = null,
        [Description(
            "The media type for the upload, such as text/markdown or application/json. Used for format inference when contentFormat is omitted later."
        )]
    ...
    [Description(
        "Append UTF-8 text to a server-managed upload session. Use this for local Markdown or JSON files instead of assuming shared server storage."
    )]
    public async Task<CallToolResult> AppendUploadChunk(
        ...
        [Description(
            "UTF-8 text chunk to append to the upload. Must not exceed maxUploadChunkBytes returned by notes_get_limits."
        )]
            string chunkText,
        ClaimsPrincipal user,
        IOptions<McpOptions> mcpOptionsAccessor,
    ...
    )
    {
        ...
        var response = new DiscardUploadToolResponse(
            uploadId,
            removed ? "discarded" : "already_absent"
        );
        await WriteUploadObservationAsync(
            auditService,
            logger,
    ...
    private static McpAuditRecord CreateUploadAuditRecord(
        ...
        string resultCode,
        string? errorCode
    ) => new(actorUserId, "user", toolName, null, null, succeeded, resultCode, errorCode);
