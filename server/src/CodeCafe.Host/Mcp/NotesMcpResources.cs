...
public sealed class NotesMcpResources
{
    ...
    [Description(
        "Recommended MCP workflow for reading, searching, and importing Markdown or TipTap JSON content."
    )]
    public TextResourceContents GetGuideResource()
    {
        var text = string.Join(
            ...
            new[]
            {
                ...
                $"- For local files or larger payloads, call `{NotesMcpToolNames.PrepareHttpUpload}` first. It returns the preferred HTTP upload request for `/api/mcp/uploads/markdown`, including auth, form fields, and runtime upload limits.",
                $"- Prefer that HTTP upload path for large Markdown so the file contents do not have to travel through MCP tool arguments. After upload, pass the returned upload id into `{NotesMcpToolNames.CreatePage}`, `{NotesMcpToolNames.UpdatePageContent}`, or `{NotesMcpToolNames.AppendBlocksToPage}`.",
                $"- Only if the client cannot perform the HTTP upload should it fall back to `{NotesMcpToolNames.GetLimits}`, `{NotesMcpToolNames.CreateUpload}`, and `{NotesMcpToolNames.AppendUploadChunk}`.",
                $"- For precise edits, use `{NotesMcpToolNames.ReplaceBlockAtIndex}`, `{NotesMcpToolNames.InsertBlocksAtIndex}`, `{NotesMcpToolNames.DeleteBlockAtIndex}`, or `{NotesMcpToolNames.ReplaceText}`. These mutate the existing TipTap document without requiring a full rewrite.",
    ...
    [Description(
        "Discover notebooks owned by the authenticated actor, including URIs for deeper notebook resources."
    )]
    ...
    [Description(
        "Discover public notebooks visible to the authenticated actor, including URIs for deeper notebook resources."
    )]
    ...
    )
    {
        ...
        NotesMcpSupport.EnsureMcpSuccess(
            NotesMcpSupport.RequireScope(user, mcpOptions.RequiredReadScopes)
        );
        var notebook = NotesMcpSupport.EnsureMcpSuccess(
            await NotesMcpSupport.RequireNotebookAsync(
                slug,
                user,
                notebookReadService,
                cancellationToken
    ...
    [Description(
        "Notebook folder and page tree for quick discovery. Use notes_list_items for archive filters, type filters, and pagination."
    )]
    ...
    [Description(
        "Page content with the stored TipTap JSON string and derived plain text for inspection before editing."
    )]
    ...
    )
    {
        ...
        var page = NotesMcpSupport.EnsureMcpSuccess(
            NotesMcpSupport.RequirePage(
                NotesMcpSupport.EnsureMcpSuccess(
                    await NotesMcpSupport.GetNotebookItemByMcpPathAsync(
                        notebook.Slug,
                        path,
                        CurrentUserClaims.GetUserId(user) ?? Guid.Empty,
                        notebookReadService,
    ...
    )
    {
        ...
        {
            "mine" => await notebookReadService.GetMyNotebooksAsync(
                actorId,
                search: null,
                cancellationToken,
                limit: 100
            ),
            "public" => await notebookReadService.GetPublicNotebooksAsync(
                search: null,
                actorId,
                cancellationToken,
                limit: 100
        ...
        var payload = new NotebookDiscoveryResourceResponse(
            ...
            notebooks.Count,
            notebooks
                .Select(notebook => new NotebookDiscoveryItem(
                    notebook.Title,
                    notebook.Slug,
                    notebook.Visibility,
                    notebook.CanEdit,
